...
function! unite#sources#reftex#converter#table#convert(linenr, line, context) "{{{
    let commands = []
    ...
    for cmd in commands
        ...
        elseif cmd.name ==# 'label'
            if !empty(a:context.source__environment_stack)
                        \ && (a:context.source__environment_stack[-1] ==# 'table'
                        \     || a:context.source__environment_stack[-1] ==# 'tabular')
                call add(candidates, {
                            \ 'kind': 'jump_list',
                            \ 'source': 'reftex',
                            \ 'word': cmd.content,
