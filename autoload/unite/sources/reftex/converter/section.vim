...
function! unite#sources#reftex#converter#section#convert(linenr, line, context) "{{{
    let commands = []
    ...
    for cmd in commands
        ...
        elseif cmd.name ==# 'label'
            if empty(a:context.source__environment_stack)
                        \ || (!empty(a:context.source__environment_stack)
                        \    && a:context.source__environment_stack[-1] ==# 'document')
                call add(candidates, {
                            \ 'kind': 'jump_list',
                            \ 'source': 'reftex',
                            \ 'word': cmd.content,
